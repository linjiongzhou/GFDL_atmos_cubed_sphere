...
! +-----------------------------------------------------------------------------+
 ...
 contains
! +---------------------------------------------+
     double precision FUNCTION POLYSVP_old (T,TYPE)
! ..................................
...
! ICE
      ...
      IF (TYPE == 1) THEN
         POLYSVP_old = (10.**(-9.09718*(273.16/T-1.)-3.56654*                &
...
! LIQUID
      ...
      IF (TYPE == 0) THEN
        POLYSVP_old = (10.**(-7.90298*(373.16/T-1.)+                        &
...
! +----------------------------------------------------------+
    ...
    Case(0)
      ...
      POLYSVP = ES1N ! [dyn/cm2]

    Case(1)
      ES2N = AA2_MY*EXP(-BB2_MY/TT)
      POLYSVP = ES2N ! [dyn/cm2]
    ...
    VR5_d = VR5
   	...
   	B8I=1./ROR
        ...
        IF(ISYM1 == 1)THEN
            ...
            sfndummy(1) = SFN11
            CALL JERTIMESC_KS(FF1in,R1D,SFNDUMMY,B11_MY,B8L,1,NKR,ICEMAX,COL)
            SFN11 = sfndummy(1)
        ENDIF
        ! ... IC
        !IF(sum(ISYM2) > 0) THEN
        ...
        if(TAU_RELAX > DTdyn) TAU_RELAX = DTdyn
...
! +-----------------------------------------------------------------------------+
 ...
 private
 public JERNUCL01_KS, LogNormal_modes_Aerosol_ACPC,LogNormal_modes_Aerosol
...
! +-----------------------------------------------------------------------------------------------------------+
	SUBROUTINE LogNormal_modes_Aerosol_ACPC(FCCNR_bl,FCCNR_ft,NKR,COL,XL,XCCN,RCCN,RO_SOLUTE,Scale_Fa,IType)

	implicit none
! ... Interface
		integer,intent(in) :: NKR,IType
		real(kind=r4size) ,intent(in) :: XL(:), COL, RO_SOLUTE, Scale_Fa
		real(kind=r8size) ,intent(out) :: FCCNR_bl(:),FCCNR_ft(:)
...
! ... Local
		...
		real(kind=r8size)  :: ccncon1, ccncon2, ccncon3, radius_mean1, radius_mean2, radius_mean3, &
							  sig1, sig2, sig3, arg11, arg12, arg13, arg21, arg22, arg23
		real(kind=r8size)  :: CONCCCNIN, FCCNR1, FCCNR2, FCCNR(NKR), DEG01, X0DROP,              &
                              XOCCN, X0, R0, RCCN_MICRON, S_KR, S(NKR), X0CCN, ROCCN(NKR),       &
	                          RO_SOLUTE_Ammon, RO_SOLUTE_NaCl, dNbydlogD_norm1(nkr), dNbydlogD_norm2(nkr), &
	                          BL_ccncon1,BL_ccncon2,FT_ccncon1,FT_ccncon2,                       &
                              BL_radius_mean1,BL_radius_mean2,FT_radius_mean1,FT_radius_mean2,   &
                              BL_sig1,BL_sig2,FT_sig1,FT_sig2


		real(kind=r8size) ,PARAMETER :: RCCN_MAX = 0.5D-4         ! [cm]
		real(kind=r8size) ,PARAMETER :: RCCN_MIN = 0.005D-4		! [cm]
		! ... Minimal radii for dry aerosol for the 3 log normal distribution
	 	real(kind=r8size) ,PARAMETER :: RCCN_MIN_3LN = 0.005D-4 ! [cm]
...
! ... Local
    ...
    if(IType == 1) then ! ... CLN modes
        ! ... 1st mode BL
        BL_ccncon1 = 1500.0
        BL_radius_mean1 = 10.0e-7
        BL_sig1 = 1.80

        ! ... 2nd mode BL
        BL_ccncon2 = 500.0
        BL_radius_mean2 = 50.0e-7
        BL_sig2 = 1.80

        ! ... 1st mode FT
        FT_ccncon1 = 450.0
        FT_radius_mean1 = 10.0e-7
        FT_sig1 = 1.80

        ! ... 2nd mode FT
        FT_ccncon2 = 150.0
        FT_radius_mean2 = 50.0e-7
        FT_sig2 = 1.80

    elseif( IType == 2) then ! ... POL modes
        ! ... 1st mode BL
        BL_ccncon1 = 12000.0
        BL_radius_mean1 = 10.0e-7
        BL_sig1 = 1.80
    ...
    ! ... BL
    arg11 = 1.0/(DSQRT(2.0D0*pi)*log10(BL_sig1))
    arg21 = 1.0/(DSQRT(2.0D0*pi)*log10(BL_sig2))

    dNbydlogD_norm1 = 0.0
    dNbydlogD_norm2 = 0.0
    do KR=1,NKR-1
        if(RCCN(KR) > RCCN_MIN_3LN .and. RCCN(KR) < RCCN_MAX) then
            arg12 = (log10(RCCN(KR)/BL_radius_mean1))**2.0
            arg13 = 2.0D0*((log10(BL_sig1))**2.0)
            dNbydlogD_norm1(kr) = arg11*exp(-arg12/arg13)*(log10(2.0)/3.0)
        endif
    enddo
    FCCNR_bl = BL_ccncon1*dNbydlogD_norm1/sum(dNbydlogD_norm1)
    do KR=1,NKR-1
        if(RCCN(KR) > RCCN_MIN_3LN .and. RCCN(KR) < RCCN_MAX) then
            arg22 = (log10(RCCN(KR)/BL_radius_mean2))**2.0
            arg23 = 2.0D0*((log10(BL_sig2))**2.0)
            dNbydlogD_norm2(kr) = arg21*exp(-arg22/arg23)*(log10(2.0)/3.0)
        endif
    enddo
    FCCNR_bl = (FCCNR_bl + BL_ccncon2*dNbydlogD_norm2/sum(dNbydlogD_norm2))/col

    ! ... FT
    arg11 = 1.0/(DSQRT(2.0D0*pi)*log10(FT_sig1))
    arg21 = 1.0/(DSQRT(2.0D0*pi)*log10(FT_sig2))

    dNbydlogD_norm1 = 0.0
    dNbydlogD_norm2 = 0.0
    do KR=1,NKR-1
        if(RCCN(KR) > RCCN_MIN_3LN .and. RCCN(KR) < RCCN_MAX) then
            arg12 = (log10(RCCN(KR)/FT_radius_mean1))**2.0
            arg13 = 2.0D0*((log10(FT_sig1))**2.0)
            dNbydlogD_norm1(kr) = arg11*exp(-arg12/arg13)*(log10(2.0)/3.0)
        endif
    enddo
    FCCNR_ft = FT_ccncon1*dNbydlogD_norm1/sum(dNbydlogD_norm1)
    do KR=1,NKR-1
        if(RCCN(KR) > RCCN_MIN_3LN .and. RCCN(KR) < RCCN_MAX) then
            arg22 = (log10(RCCN(KR)/FT_radius_mean2))**2.0
            arg23 = 2.0D0*((log10(FT_sig2))**2.0)
            dNbydlogD_norm2(kr) = arg21*exp(-arg22/arg23)*(log10(2.0)/3.0)
        endif
    enddo
    FCCNR_ft = (FCCNR_ft + FT_ccncon2*dNbydlogD_norm2/sum(dNbydlogD_norm2))/col

    !FCCNR_out = FCCNR_bl/col
	CONCCCNIN = col*sum(FCCNR_bl(:))
	print*,'CONCCCNIN_bl',CONCCCNIN
	CONCCCNIN = col*sum(FCCNR_ft(:))
    print*,'CONCCCNIN_ft',CONCCCNIN
...
! +--------------------------------------------+
! +----------------------------------------------------------------------------------------------------+
    SUBROUTINE LogNormal_modes_Aerosol(FCCNR_CON,FCCNR_MAR,NKR,COL,XL,XCCN,RCCN,RO_SOLUTE,Scale_Fa,IType)
...
! ... Local
    ...
    arg21 = ccncon2/(DSQRT(2.0D0*pi)*log(sig2))
    arg31 = ccncon3/(DSQRT(2.0D0*pi)*log(sig3))
                 ...
                 IF (DIFFU.NE.0)THEN
                   ...
                   IF (DIFFU == 1)THEN
                    ...
                    ! +------------------------------------------+
                         ...
                         icenucl_bf = 3.0*col*( sum(ff2in(:,1)*(xi(:,1)**2.0)) +  &
                        ...
                        icenucl_af = 3.0*col*( sum(ff2in(:,1)*(xi(:,1)**2.0)) +  &
                                                sum(ff2in(:,2)*(xi(:,2)**2.0)) +  &
                                                sum(ff2in(:,3)*(xi(:,3)**2.0)) )/rhocgs(I,K,J)
                        del_cldnucl_sum =  del_cldnucl_sum + (cldnucl_af - cldnucl_bf)
                        del_icenucl_sum =  del_icenucl_sum + (icenucl_af - icenucl_bf)
                      END IF
                     ...
                     ds_bf = 3.0*col*( sum(ff3r*(xs**2.0)) + sum(ff4r*(xg**2.0)) + sum(ff5r*(xh**2.0)) )/rhocgs(I,K,J)
                     ...
                     IF(ISYM1==1 .AND. ((TT-273.15)>-0.187 .OR.(sum(ISYM2)==0 .AND. &
                      ...
                      ELSE IF(ISYM1==0 .AND. (TT-273.15)<-0.187 .AND. &
                        ...
                        ds_af = 3.0*col*( sum(ff3r*(xs**2.0)) + sum(ff4r*(xg**2.0)) + sum(ff5r*(xh**2.0)) )/rhocgs(I,K,J)
                        del_ce_sum = del_ce_sum + (ce_af - ce_bf)
                        del_ds_sum = del_ds_sum + (ds_af - ds_bf)
                    END IF ! DIFF.NE.0
                END IF 	! DIFFU.NE.0
...
! --------------------------------------------------------------+
        IF (JIWEN_FAN_MELT == 1 .and. T_NEW(i,k,j) > 273.15 .and. ICEPROCS == 1) THEN
               ...
               CALL J_W_MELT(FF1R,XL,FF2R,XI,FF3R,XS,FF4R,XG,FF5R,XH, &
                             T_NEW(I,K,J),DT,rhocgs(I,K,J),COL,ICEMAX,NKR,lh_mlt)

               mlt_af = 3.0*col*( sum(ff3r*(xs**2.0)) +  &
                                  sum(ff4r*(xg**2.0)) + sum(ff5r*(xh**2.0)) )/rhocgs(I,K,J)
        END IF
...
! +----------------------------+
 ...
 ! -----------------------------------------------------------+
    ...
    LH_rate(i,k,j) = LH_rate(i,k,j) +  ((lh_ce_1 + lh_ce_2 + lh_ce_3) + lh_frz + lh_mlt + lh_rime + lh_homo + lh_ice_nucl)/dt
    CE_rate(i,k,j) = CE_rate(i,k,j) +    del_ce_sum/dt
    DS_rate(i,k,j) = DS_rate(i,k,j) +    del_ds_sum/dt
    Melt_rate(i,k,j) = Melt_rate(i,k,j) + (mlt_af - mlt_bf)/dt
...
! ... Graupel or Hail
 ...
 if (.NOT. ALLOCATED(FCCNR_bl)) ALLOCATE(FCCNR_bl(NKR_aerosol))
 if (.NOT. ALLOCATED(FCCNR_ft)) ALLOCATE(FCCNR_ft(NKR_aerosol))
 ...
 if (.NOT. ALLOCATED(FCCN)) ALLOCATE(FCCN(NKR_aerosol))
    ...
    ENDIF
 	...
 	IF(ILogNormal_modes_Aerosol_ACPC == 1)THEN
 		! ... Initializing the FCCNR_ACPC , 1 - CLN, 2 - POL
 		FCCNR_bl = 0.0 ; FCCNR_ft = 0.0
 		Scale_CCN_Factor = 1.0
 		XCCN = 0.0
 		RCCN = 0.0
 ...
 ! -------------------------------------------------------------+
  SUBROUTINE FREEZ(FF1,XL,FF2,XI,FF3,XS,FF4,XG,FF5,XH, &
			             ...
			             BFREEZMAX,KRFREEZ,ICEMAX,NKR,lh_frz)

 	  IMPLICIT NONE

 	    INTEGER KR,ICE,ICE_TYPE
      REAL COL,AFREEZMY,BFREEZMY,BFREEZMAX,lh_frz
 ...
 ! ----------------------------------------------------------------+
  SUBROUTINE J_W_MELT(FF1,XL,FF2,XI,FF3,XS,FF4,XG,FF5,XH &
                     ,TIN,DT,RO,COL,ICEMAX,NKR,lh_mlt)
 ...
 SUBROUTINE ONECOND2 &
 					 ...
 					 & ,COL,DTCOND,ICEMAX,NKR &
 					 & ,ISYM1,ISYM2,ISYM3,ISYM4,ISYM5, &
 					 	Iin,Jin,Kin,W_in,DX_in,Itimestep,lh_ce_2)
