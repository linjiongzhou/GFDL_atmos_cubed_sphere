...
! lookup table values for rain number- and mass-weighted fallspeeds and ventilation parameters
 ...
 logical :: log_ModAdvZitot = .true.  ! switch to use scaled Zitot advection and mixing (Zitot*Nitot)^(1/2)
 logical :: log_full3Mom    = .false. ! switch to turn on fully 3-moment ice
 ...
 namelist / p3_mp_nml / log_trplMomI, log_liqfrac, clbfact_dep, clbfact_sub, debug_on, scpf_on, &
                        ...
                        rparam, log_liqsatadj, log_predictSsat, log_ModAdvZitot, log_full3Mom
...
! Local variables and parameters:
 ...
 character(len=1024), parameter :: version_p3                    = '5.4.5'
...
!----- input/ouput arguments:  ------------------------------------------------------------!
 ...
 real, intent(inout), dimension(ni,nk)  :: qr                    ! rain  specific ratio, mass            kg kg-1
 real, intent(inout), dimension(ni,nk)  :: nr                    ! rain  specific ratio, number          #  kg-1
 real, intent(inout), dimension(ni,nk)  :: qc_m                  ! cloud specific ratio, mass t-         kg kg-1
 real, intent(inout), dimension(ni,nk)  :: qr_m                  ! rain  specific ratio, mass t-         kg kg-1
...
! change from input/output variables to local variables
 ...
 real, dimension(ni,nk)  :: diag_vis1             ! visibility through liquid fog       m
 real, dimension(ni,nk)  :: diag_vis2             ! visibility through rain             m
...
!----- local variables and parameters:
 ...
 real, dimension(ni,nk)  :: hgtt                ! layer-top height                        m
 real, dimension(ni,nk)  :: DZ                  ! difference in height between levels     m
 real, dimension(ni,nk)  :: ssat                ! supersaturation
 real, dimension(ni,nk)  :: tmparr_ik           ! temporary array (for optimization)
 real, dimension(ni,nk)  :: qqdelta,ttdelta     ! for sub_stepping
...
!----------------------------------------------------------------------------------------!
   ...
   if (kount.eq.1.and.(.not.warm_start)) then
      temp_m = temp
      qvap_m = qvap
      qc_m = qc
      qr_m = qr
   ...
   ! External forcings are distributed evenly over steps
   qqdelta = (qvap-qvapm1) / float(n_substep)
   ttdelta = (ta-ta_m) / float(n_substep)
   ! initialise for the 1st substepping
   qvap = qvapm1
   ta = ta_m
 ...
 ! note: code for prediction of ssat not currently avaiable, thus array is to 0
  ...
  !air pressure, layer-centered height, and layer thickness:
   ...
   enddo
   hgtt(:,kbot) = -delz(:,kbot)
   hgtc(:,kbot) = -0.5*delz(:,kbot)
   do k = kbot-1,ktop,kdir
      hgtt(:,k)= hgtt(:,k+1)-delz(:,k)
      hgtc(:,k)= hgtt(:,k+1)-0.5*delz(:,k)
   enddo
  ...
  !decompose full ice arrays back into individual category arrays:
   ...
   nitot_1(:,:) = nitot(:,:,1)
   birim_1(:,:) = birim(:,:,1)
   if (log_trplMomI) zitot_1(:,:) = zitot(:,:,1)
   if (log_liqFrac) qiliq_1(:,:) = qiliq(:,:,1)
   where (qitot_1(:,:) >= SMALL_ICE_MASS)
      diag_effi_1(:,:) = diag_effi(:,:,1)
   elsewhere
      diag_effi_1(:,:) = 0.
 ...
 end subroutine mp_p3_wrapper_shield
    ...
    ! note 'f1pr' values are normalized, so we need to multiply by N
      ...
      if (log_LiquidFrac) then
         ...
         if ((qitot(i,k,iice)-qiliq(i,k,iice)).ge.qsmall .and. t(i,k).gt.273.15) then
             ...
             if (log_3momentIce) then
                zimlt(iice) = -((f1pr24*f1pr32+f1pr25*f1pr33*sc**thrd*(rhofaci(i,k)*rho(i,k)/mu)**0.5)*((t(i,k)-   &
                            273.15)*kap-rho(i,k)*xxlv(i,k)*dv*(qsat0-Qv_cld(k)))*2.*pi/xlf(i,k)+   &
                            dum)
             endif
          endif
...
! ice
       do iice = 1,nCat
          ...
          sources = qitot(i,k,iice) + (qidep(iice)+qinuc(iice)+qrcol(iice)+qccol(iice)+          &
                    qrhetc(iice)+qrheti(iice)+qchetc(iice)+qcheti(iice)+qrmul(iice)+qcmul(iice)+ &
